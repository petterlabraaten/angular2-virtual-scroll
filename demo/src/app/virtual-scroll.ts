...
class DefaultVirtualScrollItemsProvider implements IVirtualScrollItemsProvider<any> {
    ...
    public getIterator(start: number, end: number): Iterator<any> {
...
export class VirtualScrollComponent implements OnInit, OnChanges, OnDestroy {
    ...
    private _itemsProvider: IVirtualScrollItemsProvider<any> = new DefaultVirtualScrollItemsProvider([]);


    @Input() set items(value: any[] | IVirtualScrollItemsProvider<any>) {
    ...
    get items(): any[] | IVirtualScrollItemsProvider<any> {
    ...
    @Output()
    update: EventEmitter<Iterator<any>> = new EventEmitter<Iterator<any>>();
    ...
    scrollInto(item: any) {
        ...
        let index: number = this._itemsProvider.indexOf(item);
        if (index < 0 || index >= this._itemsProvider.getLength()) return;
    ...
    private calculateDimensions() {
        ...
        let itemCount = this._itemsProvider.getLength();
    ...
    private calculateItems() {
        ...
        const topPadding = (items == null || this._itemsProvider.getLength() === 0) ? 0 : (d.childHeight * Math.ceil(start / d.itemsPerRow) - (d.childHeight * Math.min(start, this.bufferAmount)));
        ...
        if (start !== this.previousStart || end !== this.previousEnd) {
            ...
            this.zone.run(() => {
                ...
                this.viewPortItems = this._itemsProvider.getIterator(start, end);
